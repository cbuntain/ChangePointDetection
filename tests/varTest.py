...
def kcdWrapper(data):
	...
	(changePoints, stats) = kernelChangeDetection(data, d=250, eta=0.7, gamma=0.005, nu=0.001, useCov=False)
...
if ( len(sys.argv) < 2 ):
	print "Usage: %s [vartype]" % (sys.argv[0])
	exit(1)

varType = int(sys.argv[1])
...
logger.info("n: %d", n)
logger.info("VAR Order: %d", varOrder)
logger.info("Cov Shift: %s", covShift)
logger.info("Mean Shift: %s", meanShift)
...
for i in range(runs):
	...
	covShiftWrapper = lambda s, i: s
	if ( covShift == True ):
		covShiftWrapper = lambda s, i: covShifter(s, i, k)
	
	meanShiftWrapper = lambda m, i: m
	if ( meanShift == True ):
		meanShiftWrapper = lambda m, i: meanShifter(m, i, k)
